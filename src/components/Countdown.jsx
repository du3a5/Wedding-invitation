...
export default function Countdown({ t }) {
  ...
  return (
    <section className="w-full py-16 px-4 bg-gradient-to-b from-[#FAF6F0] via-[#F7ECE1] to-[#FAF6F0] flex flex-col items-center justify-center text-center overflow-hidden">
      <ScrollReveal className="max-w-2xl w-full mx-auto">
        
        {/* Heading with Full Tashkeel in Thmanyah Font */}
        ...
        <div className="grid grid-cols-4 gap-3 sm:gap-6 max-w-lg mx-auto">
          {timeUnits.map((unit, idx) => (
            ...
            >
              {/* Digit Box with Flip Effect */}
              <div className={`w-full flex items-center justify-center ${unit.isFlipping ? 'animate-flip-tick' : ''}`}>
                <span className="font-thmanyah text-3xl sm:text-5xl font-bold text-[#580E18]">
                  {String(unit.value).padStart(2, '0')}
                </span>
              </div>

              {/* Decorative Thmanyah Font Unit Labels */}
              <span className="font-thmanyah text-sm sm:text-base text-[#8C6D33] font-bold mt-2 tracking-wide">
                {unit.label}
              </span>
            </div>
          ))}
